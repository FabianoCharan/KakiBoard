...
namespace KakiBoard.SharedKernel.Validation
{
    public static class AssertionConcern
    {
        ...
        public static DomainNotification AssertPasswordIsValid(string password, string message)
        {
            // Do not validate if no PASSWORD is provided
            // Asssert if password is strong
            // You can call AssertNotEmpty before this if you want
            if (String.IsNullOrEmpty(password))
                return null;

            var regex = @"^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).{6,20}$";

            return (!Regex.IsMatch(password, regex, RegexOptions.IgnoreCase))
                ? new DomainNotification("AssertPasswordIsInvalid", message)
                : null;
        }
    }
}
